        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-gray-600 text-sm">
              {[profile.avatar.demographics.age, profile.avatar.demographics.gender, profile.avatar.demographics.location, profile.avatar.demographics.income].filter(Boolean).join(', ')}
            </p>
          </div>
          <div>
            <h4 className="text-sm font-medium text-gray-700">Interests</h4>
            <p className="text-gray-600 text-sm">{profile.avatar.psychographics.interests.join(', ')}</p>
          </div>
          <div>
            <h4 className="text-sm font-medium text-gray-700">Pain Points</h4>
            <ul className="list-disc list-inside text-gray-600 text-sm space-y-1">
              {profile.avatar.psychographics.painPoints.map((point: string, idx: number) => (
                <li key={idx}>{point}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc list-inside text-gray-600 text-sm space-y-1">
              {profile.avatar.psychographics.desires.map((desire: string, idx: number) => (
